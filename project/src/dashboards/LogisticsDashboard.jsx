...
import LogisticsHome from "./logistics/LogisticsHome"; // import home page

const LogisticsDashboard = ({ previewMode = false }) => {
  const { user } = useAuth();
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="w-72 bg-white border-r hidden md:block">
        <div className="p-6 border-b">
          ...
          <p className="text-sm text-gray-500">{user?.email || "logistics@supply.com"}</p>
        </div>
        ...
        <div className="p-4 mt-auto">
          <small className="text-xs text-gray-400">SupplySphere • Logistics</small>
        </div>
      </aside>
      ...
      <main className="flex-1 p-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          {previewMode ? <LogisticsHome /> : <Outlet />}
        </div>
      </main>
    </div>
  );
